...
class SwiftAPIGenerator extends language_1.SwiftGenerator {
    ...
    classDeclarationForOperation(operation, outputIndividualFiles) {
    ...
    structDeclarationForFragment({ fragmentName, selectionSet, source }, outputIndividualFiles) {
            ...
            if (graphql_1.isCompositeType(unmodifiedFieldType)) {
                ...
                if (graphql_2.isList(type)) {
                    ...
                    this.withinBlock(() => {
                        ...
                        this.printOnNewline(`return ${this.helpers.mapExpressionForType(type, isConditional, expression => `${structName}(unsafeResultMap: ${expression})`, expression, "ResultMap", structName)}.filter`);
                        this.withinBlock(() => this.printOnNewline(`${structName}.possibleTypes.contains($0.__typename)`), "({", "})");
                    });
                    this.printOnNewline("set");
                    this.withinBlock(() => {
    ...
    enumerationDeclaration(type) {
        ...
        this.printOnNewline(`public enum ${name}: RawRepresentable, Equatable, Hashable, CaseIterable, Apollo.JSONDecodable, Apollo.JSONEncodable`);
        this.withinBlock(() => {
        ...
        this.structDeclaration({ structName, description: description || undefined, adoptedProtocols }, outputIndividualFiles, () => {
            ...
            for (const { name, propertyName, typeName, description, isOptional } of properties) {
                ...
                this.withinBlock(() => {
                    ...
                    this.withinBlock(() => {
                        if (isOptional) {
                            this.printOnNewline(`return graphQLMap["${name}"] as? ${typeName} ?? .none`);
                        }
                        else {
